...
interface LocationsInterface {
  ...
  left: number,
  label: string,
  spawns: { label: string, x: number, y: number, z: number }[]
}
...
const App: React.FC = () => {
  ...
  return (
    <div className={`nui-wrapper ${show}`} style={{ visibility: showHidden ? 'hidden' : 'visible' }}>
      ...
      <div className='last-location'><Button color='info' style={{ width: '12.5rem', height: '3.125rem', border: '0.125rem solid hsl(220, 17%, 37%)', top: '60px', left: '30px', fontSize: '1.25rem', fontFamily: 'DevX32', background: 'hsl(0, 0%, 13%)' }} variant='contained' onClick={lastLocation}>Last Location</Button></div>
    </div>
  );
}
